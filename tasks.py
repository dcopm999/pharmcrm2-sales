...
def coverage(c):
    ...
    """
    c.run("coverage run --source pharmcrm2-sales runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/pharmcrm2-sales.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ sales")
...
def lint(c):
    ...
    """
    c.run("flake8 pharmcrm2-sales tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        "git tag -a {version} -m 'New version: {version}'".format(
            version=pharmcrm2_sales.__version__
        )
    )
    c.run("git push --tags")
